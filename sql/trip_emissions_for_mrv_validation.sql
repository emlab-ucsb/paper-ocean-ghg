...
trip_level_filtered AS (
    SELECT 
        ...
        v.trip_type
    FROM 
        `world-fishing-827.proj_ocean_ghg.trip_level_emissions_{run_version_ais}` AS t
    JOIN 
...
valid_ssvid AS (
    SELECT 
        ...
        imo_number
    FROM 
        `world-fishing-827.proj_ocean_ghg.vessel_info_{run_version_ais}` AS vi
    JOIN 
        `world-fishing-827.proj_ocean_ghg.eu_validation_data_{run_version_ais}` AS evd
    ON 
