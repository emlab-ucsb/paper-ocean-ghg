WITH
  ...
  # We only care about non-missing ratios
  WHERE
...
from (select all_data.*,
             ROW_NUMBER() over (PARTITION BY CAST(lon_bin_from AS STRING),CAST(lat_bin_from AS STRING),fishing,time,length_size_class_percentile ORDER BY distance_m ASC) as nearest_neighbor_rank
      from all_data
     ) t
