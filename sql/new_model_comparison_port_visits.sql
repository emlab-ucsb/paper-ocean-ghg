WITH
  ...
  FROM
    `world-fishing-827.proj_ocean_ghg.port_visit_level_emissions_v20250701`
  JOIN(SELECT visit_id, end_timestamp FROM `world-fishing-827.proj_ocean_ghg.port_visit_info_v20250701`)
  USING(visit_id)
  GROUP BY
    year),
  old_table AS(
  ...
  FROM
    `world-fishing-827.proj_ocean_ghg.port_visit_level_emissions_v20241121`
  JOIN(SELECT visit_id, end_timestamp FROM `world-fishing-827.proj_ocean_ghg.port_visit_info_v20241121`)
  USING(visit_id)
  GROUP BY
    year)
SELECT
