...
summarize_dark_fleet_model_results_emissions <- function(dark_fleet_model_results, time_extrapolation  = "YEAR" ){
  
  if (time_extrapolation == "YEAR") {
    
    dark_fleet_model_results %>%
      mutate(year = lubridate::year(time)) %>%
      dplyr::select(-c(time,ratio_dark_to_ais_detections,global_time_ratio_dark_to_ais_detections,number_dark_detections,number_ais_detections, null_ratio)) %>%
      pivot_longer(-c(year, lon_bin, lat_bin, fishing, length_size_class_percentile),
                   names_to = "pollutant",
                   values_to = "emissions_mt") %>%
      mutate(dark = ifelse(stringr::str_detect(pollutant,"dark"),TRUE,FALSE),
             pollutant = stringr::str_remove_all(pollutant,"emissions_") %>%
               stringr::str_remove_all("_mt")  %>%
               stringr::str_remove_all("_dark") %>%
               stringr::str_to_upper()) %>%
      group_by(year, lat_bin, lon_bin, pollutant, dark, fishing, length_size_class_percentile) %>%
    ...
    dark_fleet_model_results %>%
      ...
      dplyr::select(-c(year,month,time,length_size_class_percentile,ratio_dark_to_ais_detections,global_time_ratio_dark_to_ais_detections,number_dark_detections,number_ais_detections, null_ratio)) %>%
      pivot_longer(-c(year_month, lon_bin, lat_bin, fishing),
                   names_to = "pollutant",
                   values_to = "emissions_mt") %>%
      mutate(dark = ifelse(stringr::str_detect(pollutant,"dark"),TRUE,FALSE),
             pollutant = stringr::str_remove_all(pollutant,"emissions_") %>%
