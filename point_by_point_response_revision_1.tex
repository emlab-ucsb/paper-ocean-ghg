...
\end{refquote}
\response Thank you for raising this issue, and for providing us an opportunity to clarify what we've done and revise the manuscript accordingly. The premise we should correct first is the suggestion that we did not obtain or use dedicated ship databases. We did, and our original text simply failed to say so, citing a single reference for the whole enterprise. Vessel characteristics in this analysis are drawn from the IMO's Global Integrated Shipping Information System (GISIS); S\&P Global Market Intelligence's Ships Dataset; the national vessel registries of 19 flag states (the United States, Canada, Australia, Russia, the Republic of Korea, Norway, Indonesia, Iceland, Taiwan, the Faroe Islands, Spain, Panama, Peru, Ecuador, Seychelles, Malaysia, Belize, Papua New Guinea and Madagascar); the EU fleet register and FAO records; and the authorized-vessel lists of thirteen regional fisheries management organizations (ICCAT, IOTC, WCPFC, IATTC, SPRFMO, CCSBT, NEAFC, NPFC, GFCM, NAFO, SIOFA, CCAMLR and SEAFO). This enumeration is now in the Methods.

Where a vessel matches one of these sources, we use its registered characteristics rather than inferring them. Of the 775,358 AIS-broadcasting vessels in the current analysis, 93,379 (12.0\%) are registered in GISIS, a further 45,743 (5.9\%) appear in at least one other registry, and 139,122 (18\%) are matched in total. We should also correct an error of our own, which the reviewer has rightly caught: the 84\% came from a field recording whether a vessel appears in any registry at all, and we presented it as though it measured whether main engine power and design speed had to be inferred. Those are different quantities, and the per-characteristic figures are in fact lower rather than higher: main engine power is available from a registry or a commercial database for 7.5\% of vessels, and design speed for 1.8\%. We have replaced the single 84\% figure with a table reporting availability for each characteristic the model requires. (On one smaller point, the 10\% in the submitted text was the share of all AIS-broadcasting vessels carrying an IMO entry, not the share of IMO-registered vessels we were able to match, which is essentially complete.)

For the remainder we must infer characteristics, and the vessels concerned are of particular types rather than a random cross-section of the fleet. The 636,236 vessels we cannot match lie largely outside the IMO-registered merchant fleet that commercial databases are built around: on our classification roughly 43\% are passenger craft, a deliberately broad class spanning yachts, small ferries and Ro-Pax, 24\% are fishing vessels, and 9\% are tugs. Those databases cover the merchant fleet excellently, which is precisely why we match it essentially completely; they do not systematically enumerate vessels of these other types, and acquiring further databases would not materially change that.

This is becoming more consequential over time rather than less. The registry-matched share of AIS-broadcasting vessels falls from 30\% in 2017 to 17\% in 2025, because AIS carriage is spreading fastest among exactly those vessels that no major registry lists. Each year, a larger share of the observable fleet can only be characterized by inference, which is why we regard the vessel-characteristics models as a contribution of this work rather than a workaround for missing data. We now report this trend in the Results.

We would close on the distinction we think matters most here. Our AIS-based fleet is defined by AIS observation, not by registry membership: every AIS-broadcasting vessel enters our analysis regardless of whether any registry lists it. Additional registries would therefore improve the precision of the characteristics we assign to some vessels, but they would not add a single vessel to our fleet, and so would not change our coverage. And no registry, however comprehensive, can reach vessels that do not broadcast AIS at all. That gap is what our S1-unmatched model addresses, and it is the source of the 21\% figure, which concerns vessels absent from AIS rather than vessels absent from registries. However the registry question is resolved, it leaves that contribution untouched. We have restated the fleet-scope argument explicitly in the Discussion and no longer imply that limited registry coverage is an inherent flaw of AIS-based methods. This connects to our response to R3.3 on the choice of AIS provider and pipeline.
