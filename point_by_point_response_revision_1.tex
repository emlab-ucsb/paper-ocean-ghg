...
code and data together has been separated into the two required sections. No text
was rewritten in the course of this reordering: the sections carry the same wording
they had before, at their new positions.
...
plotting library applies by default. Because every figure in this manuscript is
generated by the code in the accompanying repository rather than assembled by hand,
this was changed at the source: the label text and size are set once for the figures
that share a common theme, so the labelling cannot drift between figures or be lost
when a figure is regenerated. Panel references in the figure captions and in the
main text were updated to match.

We also checked the lettering inside the figures against the same guidance. All of
it was already sentence case, with one exception: the facet headings on the
classification-performance figures were title case (``Inside Footprint'', ``Outside
Footprint''), and these now read ``Inside footprint'' and ``Outside footprint''.
