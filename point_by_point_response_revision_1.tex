...
Finally, our S1-unmatched model estimates emissions for each 1x1 degree pixel and month. Although the emissions regression is trained across all imaged pixel-months, it conditions on each pixel-month's own detections and on spatial, temporal and AIS reception features, so the relationship it applies can differ between places and times rather than being a single global ratio. If vessels operating in the same place and time have similar characteristics, the assumption will hold.
...
Finally, the new double-counting analysis, which quantifies an upward bias of at most 2.4\% of our fused total, is described in the Methods and quoted in full in our response to R2.4.
