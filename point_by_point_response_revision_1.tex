...
\response We have reordered the manuscript to follow the required structure. The
Methods section now follows the Discussion directly, and the Data Availability and
Code Availability sections sit immediately below it, ahead of the References. The
back matter that follows the References is now ordered Acknowledgements, Funding,
Author Contributions, Competing Interests. The statement that previously covered
code and data together has been separated into the two required sections.

\changes Section order in \texttt{main.tex} is now: Title, Author list,
Affiliations, Abstract, Introduction, Results, Discussion, Methods, Data
Availability, Code Availability, References, Acknowledgements, Funding, Author
Contributions, Competing Interests, followed by the Supplementary Information.
\todo{three items in this list still need author text, and are flagged in the .tex:
(i) the Acknowledgements section is empty now that the Climate TRACE sentence sits
under Funding, where the required order puts it; (ii) the Author Contributions
statement is still the CRediT-style list and needs rewriting as one paragraph of
descriptive sentences, as the editor asks; (iii) Results carries no subheadings yet,
where the required order asks for one level. Also note the manuscript opens with a
Summary section that is not in the editor's list, and that main-text figures remain
embedded at their first citation with their legends attached rather than collected
as a legends-only section at the end --- confirm which the editor prefers.}
...
\response All multi-panel figures have been relabelled. Panel labels are now
lower-case bold letters (a, b, c, \ldots) set at the same type size as the rest of
the type in each figure, in place of the upper-case labels at a larger size that the
plotting library applies by default.

We also checked the lettering inside the figures against the same guidance.

\changes Eighteen panel-label sites across thirteen multi-panel figures were
changed from upper-case to lower-case bold letters, and their type size set to match
each figure's own text. Panel references were lower-cased throughout the figure
captions and the main text. The title-case facet headings on the
classification-performance figures were changed to sentence case.

\point{E4 --- Colour-vision-deficiency-safe figure colours}
% category: figures | assignee: Gavin
% Avoid red/green discrimination; use colour-safe palettes.
\response We audited every figure in the manuscript rather than inspecting them by
eye. Each categorical palette was simulated for deuteranopia, protanopia and
tritanopia using the Machado et al. (2009) severity-1.0 model, and every pair of
colours within a palette was then measured in CIE $\Delta E_{2000}$. For a
categorical key the number that matters is the smallest pairwise distance, since
that is the pair a reader is most likely to confuse.

Most of the manuscript already met the standard, because the figures draw their
colours from palettes built for this purpose rather than chosen by hand: the
Okabe-Ito palette for the data-source series that carry Figures 1, 2 and S1;
Paul Tol's \emph{Safe} palette for categorical keys; the viridis family
(\texttt{magma}, \texttt{mako}) for the gridded emissions maps; and Crameri's
\texttt{scico} palettes for the diverging change maps. These are perceptually
uniform and colour-vision-deficiency-safe by construction, and wherever a figure
needed arbitrary colours we have taken them from one of these packages rather
than inventing them. The training/testing map already used the turquoise-and-red
pairing the editor's guidance suggests. The audit also confirmed what these
choices are worth: the gridded maps vary monotonically in lightness, so they
carry their information without relying on hue discrimination at all, and the
diverging change map holds 20.4 $\Delta E$ between its arms under protanopia.

The audit did find five palettes that did not meet the standard, all of them
places where colours had been chosen arbitrarily, and four more that were
marginal. We have changed all nine:

\begin{itemize}\setlength\itemsep{2pt}
\item \textbf{Ocean trends (Figure 4a)} used ColorBrewer \emph{Dark2}, whose
green and orange --- Arctic against Pacific --- were 3.4 $\Delta E$ apart under
deuteranopia. This is now Tol's \emph{Safe} palette, at 17.7.
\item \textbf{Fleet composition and fleet growth (Supplementary)} spread a
\emph{Spectral} ramp across vessel classes. Spectral runs red--yellow--green and
put neighbouring classes 1.4 $\Delta E$ apart under deuteranopia. These now use
\emph{Safe}, at 12.8 (deuteranopia) and 11.8 (protanopia).
\item \textbf{Vessel length bins (Supplementary)} used the \texttt{turbo}
rainbow, at 2.5 $\Delta E$. These bins are ordered, so they now use a sequential
\texttt{mako} ramp, which contains no red at all.
\item \textbf{AIS receiver type (Supplementary)} used ColorBrewer \emph{Set2},
whose pink and blue-purple were 1.6 $\Delta E$ apart under protanopia; now
\emph{Safe}, at 19.8.
\item \textbf{Multi-sector inventory comparisons (Supplementary)} encoded the
inventory as a hue family and the scope as lightness within it, which placed the
EDGAR and CEDS series 4.0 $\Delta E$ apart under protanopia. Changing hues does
not fix this --- three dark family endpoints converge whatever hues are chosen ---
so the inventory is now encoded twice, in hue and in line style, and a reader who
cannot separate the families by colour can still separate them by line. The same
redundancy was added to the registry-split figure, where three shades of one blue
carried a nested split.
\item Four further adjustments removed marginal pairs: the EDGAR all-sector line
in Figure 4b (6.4 $\rightarrow$ 24.9), the SEIM inventory line (7.0
$\rightarrow$ 12.5), and the lightness spacing within the EDGAR scope ramp (8.2
$\rightarrow$ 16.1).
\end{itemize}

Two residuals are worth stating plainly. The vessel-class figures draw eleven and
twelve categories respectively; no qualitative palette keeps that many colours
separable, and while the red--green axis is now comfortably clear (12.8 and 11.8),
one pair remains at 6.0 under tritanopia, which is far rarer and is not the
discrimination the guidance is concerned with. Similarly, ten ordered length bins
cannot all be far apart on any single sequential ramp; the ordering itself, and
the label each line carries at its right-hand end, is what separates neighbours
there. In both cases the affected figures also label their classes directly, so
colour is not the only channel carrying the identity.

\changes Palettes were changed in five figures and refined in four more, all at
the source in the analysis code, so the choices cannot drift when figures are
regenerated. Line style was added as a redundant encoding in the multi-sector and
registry-split figures.
...
\end{refquote}
\response We thank the reviewer for bringing up this important point to clarify. We believe our model framework has already addressed the majority of the reviewer's concern. Specifically, for vessel sizes, we have already binned the vessels into fine length groups and dropping small non-broadcasting vessels below the smallest AIS bin, so we are not imposing this assumption to vessels of very different lengths. For operational behavior, our detections are already classified as fishing and nonfishing vessels, which is major factor determining operational behavior and emissions factors. For vessels of the same size and type, the fact that we are using Sentinel-1 imagery further helps reducing the variability inside the group, because SAR has limited ability of detecting wooden and fiberglass vessels. This means we are not applying the emission behavior of a small industrial fishing boat to a wooden small-scale fishing boat or fiberglass recreational boat of similar size.

In addition, non-broadcasting vessels in this study, defined as S1 vessel detections unmatched with AIS, could correspond to \textit{either} vessels that do not carry an AIS transponder at all; \textit{or} to vessels that carry AIS transponders but temporarily non-broadcasting or whose signal was not received. Therefore, AIS-broadcasting and non-broadcasting vessels are not necessarily two distinct fleets, but can be two transient states. In these cases, the `relationship between SAR-detected vessel density and emissions' is exactly the same between the two transient states, because the two states represent the exact same set of vessels.
\changes
...
\end{refquote}
\response
Thank you for this clarifying comment. Since the initial submission, we have developed machine learning models to predict engine type (oil, gas, or turbine), construction year, and engine speed (RPM) for each AIS-broadcasting vessel. This allows us to now more precisely assign emissions factors, including the ability to assign factors for different IMO NOx tiers and engine speeds. When a vessel's engine type cannot be resolved to a single emissions factor, we now use the predicted engine type to narrow the applicable emission factor columns and average across them. Although we attempted developing a machine learning model to predict fuel type as well, we were unable to achieve reasonable predictive performance using the proprietary vessel registry we had access to.

\changes

We’ve added this new information in Section 4.1.2 on estimating NOx emissions. We have additionally added a description of the engine type, construction year, and engine speed machine learning models and their performance to the methods section. We also describe the emission-factor averaging approach to the methods section. We finally  added a paragraph to the Discussion/Limitations noting the current inability to resolve fuel type and how additional training data could address this in future work.
...
\changes We added a dedicated section in the supplementary materials that includes a summary table of performance metrics alongside detailed breakdowns for each model with the information above. We also made minor adjustments to the main text to point readers towards the newly written section.
...
\end{refquote}
\response We apologize for the lack of clarity in our original text. To clarify, we successfully matched virtually all IMO-registered vessels to registry data (not 10\% as suggested). What we meant to say is that only roughly 13\% of vessels analyzed are IMO registered (~113,000 out of ~865,000) and therefore we were able to find at least some characteristic information about them in S\&P Global Market's Ship Dataset or other open registries. We were able to find characteristic information for an additional ~48,000 vessels that were not IMO registered. 
\changes We have rewritten the relevant text for clarity and added a supplementary table breaking down the registry coverage by characteristic. 
...
\changes In the main results text when describing AIS-broadcasting emissions by different vessels classes, we have added the following (Lines XX-YY)

\begin{quote}
 We note that not all AIS messages are broadcast by vessels; we are therefore careful to exclude all non-vessel AIS messages from our emissions estimates. Roughly 5\% of AIS-enabled signals are actually attributable to buoys, FADs, navigation aids, offshore structures, fishing gear, and other objects. These are classified as a separate `gear' class by our vessel classification model, and are not included in this analysis. 
\end{quote}

In the supplementary section 1.7 (Lines XX-YY), we also now say:

\begin{quote}
Note that nearly half of AIS-broadcasting devices are likely associated with non-vessel objects (such as buoys, offshore structures, FADs, etc). These objects are identified as their own vessel class `gear' by the primary GFW vessel class model, and are excluded from the emissions estimates in this paper.
\end{quote}

In Section 4.2.1, when describing our S1 vessel detection processing procedure (Lines XX-YY), we say:

\begin{quote}
The data filtering involved in the GFW vessel detections include removing detections within a 1-km buffer from the shoreline to reduce false positives from land-based objects due to inaccurately defined and dynamic coastlines, repeated detections of potentially fixed objects such as offshore infrastructure, detections from icy areas, moving vehicles close to shore, and other radar ambiguities
\end{quote}
...
approach.
\end{refquote}
\response We thank the reviewer for the suggestion. We added a brief description of the S1 imagery dataset, including the resolution, the all-weather imaging ability, and the revisit frequency.
\changes We added a brief description of the imagery after mentioning the S1 SAR imagery, as suggested by the reviewer. In Lines XX-YY of the introduction, we now say:

\begin{quote}
The 20m-resolution S1 SAR imagery allows us to detect and estimate the lengths of most vessels larger than 15 m regardless of whether they broadcast AIS, cloud cover, weather conditions, or solar illumination (Paolo et al. 2024). The two S1 satellites active during our study period (full period for S1A, 2017-2021 for S1B) operate in a polar, sun-synchronous orbit, providing a global revisit frequency of 6 to 12 days.
\end{quote}
