...
\end{refquote}
\response Thank you for raising these important and related issues, and for providing us an opportunity to clarify what we've done and revise the manuscript accordingly. Our original sentence did not describe what we had actually measured: the 84\% came from a single field recording whether a vessel appears in any registry we could match to, not from any measurement of whether main engine power and design speed specifically had to be inferred. Presenting the former as though it were the latter was our error, and we have rewritten the passage. The exact numbers have also changed since submission, because the fleet is now defined over 2017--2025 and the vessel-characteristics database has been rebuilt: of the 775,358 AIS-broadcasting vessels in the current analysis, 93,379 (12.0\%) are registered in the IMO's Global Integrated Shipping Information System (GISIS), a further 45,743 (5.9\%) appear in at least one other vessel registry, and 139,122 (18\%) are matched in total. We should also correct one small misreading: the 10\% in the submitted text was the share of all AIS-broadcasting vessels carrying an IMO entry, not the share of IMO-registered vessels we were able to match, which is essentially complete.

We should add that these counts pool the whole 2017--2025 period, and that the within-year picture is less favorable to us: because registered vessels broadcast persistently while unregistered vessels appear intermittently and change MMSI, the registry-matched share within individual years falls from 30\% of broadcasting vessels in 2017 to 17\% in 2025. We now report this in the Results. We would stress, though, that this trend is itself part of our argument rather than only a caveat against it. AIS carriage is spreading fastest among exactly those vessels that no major registry lists, so the share of the observable fleet that can only be characterized by inference grows every year. Accurate inference of missing vessel characteristics therefore becomes more important over time, not less, and an inventory that depends on registry lookup will describe a shrinking fraction of the vessels actually at sea.

On the substance of the concern, that we did not obtain or use dedicated ship databases: we did, and we accept that the submitted text did not make this visible. Vessel characteristics are drawn from GISIS; from S\&P Global Market Intelligence's Ships Dataset; from the national vessel registries of 19 flag states (USA, CAN, AUS, RUS, KOR, NOR, IDN, ISL, TWN, FRO, ESP, PAN, PER, ECU, SYC, MYS, BLZ, PNG and MDG); from the EU fleet register and FAO records; and from the authorized-vessel lists of thirteen regional fisheries management organizations (ICCAT, IOTC, WCPFC, IATTC, SPRFMO, CCSBT, NEAFC, NPFC, GFCM, NAFO, SIOFA, CCAMLR and SEAFO). This enumeration is now in the Methods.

Coverage is nevertheless low, and the reason is how our fleet is defined rather than which databases we hold. Commercial ship databases are built around the IMO-registered merchant fleet, and for that fleet their coverage is excellent, which is precisely why we match it essentially completely. Our fleet is instead defined by AIS observation, and the 636,236 vessels we cannot match lie largely outside that merchant fleet: on our classification roughly 43\% are passenger craft, a deliberately broad class spanning yachts, small ferries and Ro-Pax, 24\% are fishing vessels, and 9\% are tugs. Acquiring further databases would not materially change this. It is also why the per-characteristic figures are lower than 84\%, not higher: main engine power is available from a registry or a commercial database for 7.5\% of vessels, and design speed for 1.8\%. We have replaced the single 84\% figure with a table reporting availability for each characteristic the model requires.

Finally, we accept the framing point. Our claim that AIS-only approaches understate emissions does not rest on our registry coverage: the 21\% figure is the share of the fused total contributed by Sentinel-1 detections that could not be matched to any AIS broadcast, and so concerns vessels absent from AIS rather than vessels absent from registries. The separate and weaker claim, that an inventory whose fleet is defined by registry membership cannot represent vessels outside those registries, is now stated explicitly as a matter of fleet scope, and we no longer imply that this is an inherent flaw of AIS-based methods. This connects to our response to R3.3 on the choice of AIS provider and pipeline.
