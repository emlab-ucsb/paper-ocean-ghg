...
edgar_comparison <- all_tranportation_data_normalized |>
  ...
  dplyr::filter(data_source %in% c("Ocean-based transportation (GFW)",
                                                 "Land-based transportation (EDGAR)")) |>
  dplyr::select(-c(emissions_co2_mt,emissions_co2_mt_starting)) |>
  tidyr::pivot_wider(values_from = emissions_co2_mt_normalized,
                     names_from = data_source) |>
  dplyr::mutate(difference = `Ocean-based transportation (GFW)`/`Land-based transportation (EDGAR)`)

iea_comparison <- all_tranportation_data_normalized |>
  ...
  tidyr::pivot_wider(values_from = emissions_co2_mt_normalized,
                     names_from = data_source) |>
  dplyr::mutate(difference = `Ocean-based transportation (GFW)`/`Land-based transportation (IEA)`)

knitr::kable(edgar_comparison,digits=3)

knitr::kable(iea_comparison,digits=3)
```
