...
annual_emissions_by_fleet_year <- purrr::map_df(seq(min(annual_ais_co2_emissions_by_vessel$year),
  ...
  dplyr::mutate(name = dplyr::case_when(stringr::str_detect(name,"emissions") ~ "CO2 emissions by AIS-broadcasting vessels",
                                        stringr::str_detect(name,"power") ~ "Total main engine power for AIS-broadcasting vessels (kW)",
                                        stringr::str_detect(name,"vessels") ~ "Number unique AIS-broadcasting vessels",
                                        TRUE ~ "Operating hours for AIS-broadcasting vessels"))
...
#| fig-cap: Annual distributions of individual vessel-level CO2 emissions (top panel); operating hours (middle panel); and main engine power (panel). The point shows the mean value, and the error bars show +/- two standard errors.
#| fig-width: 7.5
#| fig-height: 5

annual_ais_co2_emissions_by_vessel |>
  # dplyr::inner_join(annual_ais_co2_emissions_by_vessel |>
  #                     dplyr::filter(year == 2015) |>
  #                     dplyr::select(ssvid), by = "ssvid") |>
   tidyr::pivot_longer(-c(year,ssvid)) |>
  dplyr::mutate(name = dplyr::case_when(stringr::str_detect(name,"emissions") ~ "CO2 emissions by individual AIS-broadcasting vessels",
                                        stringr::str_detect(name,"power") ~ "Main engine power for individual AIS-broadcasting vessels (kW)",
                                        TRUE ~ "Operating hours for individual AIS-broadcasting vessels")) |>
