...
```{r}
#| label: fig-annual-emissions-vessels-by-ais-receiver-type
#| fig-cap: Annual total CO2 emissions (metric tonnes) and total number of distinct vessels disaggregated by AIS receiver type (terrestiral, satellite, or dynamic) and total aggregated across receiver types.
...
annual_global_emissions_by_receiver_type |>
  ...
  tidyr::complete(
    receiver_type,
    tidyr::nesting(year),
    fill = list(emissions_co2_mt = 0, n_distinct_ssvid = 99)
  ) |>
...
```{r}
#| label: fig-annual-emissions-vessels-by-ais-receiver-type-top-flags
#| fig-cap: Annual total CO2 emissions (metric tonnes) and total number of distinct vessels disaggregated by AIS receiver type (terrestiral, satellite, or dynamic) and total aggregated across receiver types, for the top 10 flags with the most emissions in 2024.
...
annual_global_emissions_by_receiver_type_and_flag |>
  ...
  tidyr::pivot_longer(cols = -c(year,receiver_type,flag)) |>
  dplyr::mutate(receiver_type = stringr::str_to_sentence(receiver_type)) |>
  ggplot(aes(x = year,
             y = value,
             color = flag)) +
  facet_grid(name~receiver_type, scales = "free_y") +
  geom_line() +
  scale_y_continuous(labels = scales::unit_format(unit = "M", scale = 1e-6)) + 
  scale_x_continuous(breaks = unique(annual_global_emissions_by_receiver_type$year)) +
  theme_plot() +
...
#| fig-cap: Spatial change in CO2 emissions between 2016 and 2024 (metric tonnes) by receiver type (dynamic, satellite, and terrestrial) and total across receiver types, at a 1x1 degree spatial resolution.
#| fig-width: 7.5
#| fig-height: 8
