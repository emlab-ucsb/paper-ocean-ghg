...
land_based_transportation_emissions_edgar <- annual_edgar_tranportation_emissions |> 
  ...
  dplyr::ungroup() |>
  dplyr::mutate(data_source = "Land-based transportation (EDGAR)")
...
land_based_transportation_emissions_iea <- annual_iea_tranportation_emissions |> 
  ...
  dplyr::summarise(emissions_co2_mt = sum(emissions_co2_mt,na.rm=TRUE)) |>
  dplyr::ungroup() |>
  dplyr::mutate(data_source = "Land-based transportation (IEA)")
...
all_tranportation_data <- land_based_transportation_emissions_edgar |>
  ...
  dplyr::bind_rows(annual_co2_emissions |> 
                     ...
                     dplyr::mutate(data_source = "Ocean-based transportation (GFW)") |>
                     dplyr::select(-fleet)) |>
...
all_transportation_color_palette = c("Ocean-based transportation (GFW)" = "steelblue4",
                                     "Land-based transportation (EDGAR)" = "olivedrab4",
                                     "Land-based transportation (IEA)" = "olivedrab3")
...
emissions_comparison_figure_absolute <- all_tranportation_data_normalized |>
  ...
  scale_color_manual(values = all_transportation_color_palette)
...
emissions_comparison_figure_normalized <- all_tranportation_data_normalized |>
  ...
  theme(panel.grid.minor.x = element_blank())+
  scale_color_manual(values = all_transportation_color_palette,
                     breaks = c("Ocean-based transportation (GFW)",
                                "Land-based transportation (EDGAR)",
                                "Land-based transportation (IEA)"))
...
edgar_comparison <- all_tranportation_data_normalized |>
  dplyr::filter(data_source %in% c("Ocean-based transportation (GFW)",
                                   "Land-based transportation (EDGAR)")) |>
  dplyr::filter(year == 2023) |>
  dplyr::select(-year) |>
  dplyr::mutate(data_source = ifelse(data_source == "Ocean-based transportation (GFW)","ocean_gfw","land_edgar"))|>
...
total_annual_emissions_by_fleet_fig <- annual_co2_emissions |>
  ...
  scale_color_manual(values = c("#7570b3","#377eb8","#e41a1c"),
